...
function AppointmentsPage({ items, reload, setModal }) {
  ...
  const shareSingleAppointment = x => {
    const kind = appointmentKind(x);
    ...
    if (x.place) {
      lines.push(
        `${kind === "supplies" ? "📦 Local de retirada" : "📍 Local"}: ${x.place}`
      );
    }

    if (x.address) {
      lines.push(`📌 Endereço: ${x.address}`);
    }

    if (x.notes) {
      lines.push(
        `${kind === "supplies" ? "📋 Itens / Obs" : "📝 Observações"}: ${x.notes}`
      );
    }

    if (x.status === "waitlist") {
      lines.push(`🔕 Avisos pausados — Fila de espera`);
    } else {
          ...
          >
            ...
            <div style={{
              ...
              <button
                type="button"
                className="soft"
                title="Compartilhar somente este agendamento"
                onClick={()=>shareSingleAppointment(x)}
                style={{
                  display:"inline-flex",
                  alignItems:"center",
                  justifyContent:"center",
